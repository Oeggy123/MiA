...
# ------------------------------------------
# Metapopulation SEIR-SEI Model with Insecticide and Seasonal Forcing
# ------------------------------------------

metapop_insecticide_model <- function(t, state, parameters) {
  with(as.list(c(state, parameters)), {
    ...
    epsilon_t <- ifelse(t >= t_intv, epsilon, 0)
    delta_v_t <- ifelse(t >= t_intv, delta_v, 0)
    alpha_eff <- (1 - epsilon_t) * alpha_t
    ...
    dSv <- logistic_birth - lambda_v * Sv - (mu_v + delta_v_t) * Sv + inflow(Sv, Mv) - outflow(Sv, Mv)
    dEv <- lambda_v * Sv - sigma_v * Ev - (mu_v + delta_v_t) * Ev + inflow(Ev, Mv) - outflow(Ev, Mv)
    dIv <- sigma_v * Ev - (mu_v + delta_v_t) * Iv + inflow(Iv, Mv) - outflow(Iv, Mv)
...
for (label in names(time_horizons)) {
  ...
  baseline <- ode(y = init, times = times, func = metapop_insecticide_model, parms = para, method = "ode45")
  ...
  scenarios <- lapply(names(timings), function(name) {
    ...
    sim <- ode(y = init, times = times, func = metapop_insecticide_model, parms = para, method = "ode45")
