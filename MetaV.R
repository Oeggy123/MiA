...
for (label in names(time_horizons)) {
  ...
  timings <- list(
    "Early (D - 30)" = max(0, first_day - 30),
    "On First Infection (D)" = first_day,
    "Delayed (D + 30)" = first_day + 30,
    "Very Delayed (D + 90)" = first_day + 90
  )
...
summary_stats <- meta_df %>%
  ...
  summarise(
    deaths_patch1 = round(max(Dh1)),
    deaths_patch2 = round(max(Dh2)),
    deaths_patch3 = round(max(Dh3)),
    
    peak_day_patch1 = time[which.max(Ih1)],
    peak_day_patch2 = time[which.max(Ih2)],
    peak_day_patch3 = time[which.max(Ih3)],
    
    peak_inf_patch1 = round(max(Ih1)),
    peak_inf_patch2 = round(max(Ih2)),
    peak_inf_patch3 = round(max(Ih3)),
