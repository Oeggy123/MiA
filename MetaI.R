...
# ------------------------------------------
# Metapopulation SEIR-SEI Model with Insecticide
# ------------------------------------------
...
metapop_insecticide_model <- function(t, state, parameters) {
  with(as.list(c(state, parameters)), {
    ...
    dSv <- logistic_birth - lambda_v * Sv - (mu_v + delta_v) * Sv + inflow(Sv, Mv) - outflow(Sv, Mv)
    dEv <- lambda_v * Sv - sigma_v * Ev - (mu_v + delta_v) * Ev + inflow(Ev, Mv) - outflow(Ev, Mv)
    dIv <- sigma_v * Ev - (mu_v + delta_v) * Iv + inflow(Iv, Mv) - outflow(Iv, Mv)
